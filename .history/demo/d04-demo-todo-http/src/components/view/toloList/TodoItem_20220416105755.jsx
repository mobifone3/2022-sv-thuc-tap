...
export default function TodoItem({ idx, id, name, isCheck, isEdit, handleCheckBoxClick, handleDeleteTodoById, handleSwitchEdit, handleKeyPress, handleOnChangeEdit }) {
  ...
  return (
    <li className={`row todo_item ${isCheck ? "bg-info text-light" : ""}`}>
      <div className="col-8">
        {isEdit ? <input value={name || ""} type="text" name="name" className="form-control" onChange={(e) => handleOnChangeEdit(e, { id, isCheck, isEdit })} onKeyPress={handleKeyPress}></input> : <p className="todo_name">{name}</p>}
      </div>
      <div className="col-4 d-flex justify-content-end">
        <div className="form-check">
          ...
          />
          {isEdit ? (
            <i className="fa-solid fa-check todo_icon_pen text-success" style={commonStyle} onClick={() => handleSwitchEdit(id, name, { id, name, isCheck, isEdit })}></i>
          ) : (
