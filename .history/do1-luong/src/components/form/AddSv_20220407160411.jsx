...
function AddSv({formData, handleOnChange }) {
  return (
    <div className="add_sv">
      {datas.map((item, idx) => (
        <InputField key={idx} label={item.label} type={item.type} name={item.name} value={formData.[item.name]} placeholder={item.placeholder} handleOnChange={handleOnChange}></InputField>
      ))}
