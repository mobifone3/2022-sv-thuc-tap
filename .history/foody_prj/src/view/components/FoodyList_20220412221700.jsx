  ...
  return (
    ...
    <ul className="foody_list">
      {foodyList.map((item, idx) => (
        <FoodyItem key={idx} title={item.title} desc={item.desc} name={item.name} cash={item.cash} rate={item.rate} vote={item.vote} review={item.review} value={item.value}></FoodyItem>
      ))}
    </ul>
  );
}
